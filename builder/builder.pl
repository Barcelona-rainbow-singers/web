...
build_home($shows, $pages);

sub build_home($shows, $pages) {
    my $dest     = $root_dir->child('index.html');
    my $template = get_template('home');
    say 'Building home';

    my $data = { shows => $shows, pages => $pages };
    $shows->sort(sub{ $a->{meta}{start_at} <=> $b->{meta}{start_at} })->each(sub($ev, $idx){
        $data->{days}{$ev->{meta}{start_at}->dmy}{date} ||= $ev->{meta}{start_at}->clone;
        push $data->{days}{$ev->{meta}{start_at}->dmy}{shows}->@*, $ev;
    });
...
sub build_shows($shows, $pages) {
    say 'Building shows:';
    my $template = get_template('show');
    $shows->each(sub($e, $idx){
        my $dest_dir = $root_dir->child('show', $e->{slug});
        $dest_dir->make_path;

        my $dest = $dest_dir->child('index.html');
        say ' - ', $e->{slug};
        $dest->spurt(encode 'UTF8', $template->process({ %$e, pages => $pages }));
    });
}

sub build_pages($pages, $shows) {
    say 'Building pages:';
